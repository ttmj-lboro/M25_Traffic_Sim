...
EMMap(1) = mean(emissions);
EMMap(2) = mean(emissions);
Fitness(1) = 0;
Fitness(2) = (TLMap(1)- TLMap(2))/TLMap(1) * 100;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for x = 2:500
...
EMMap(x+1) = mean(emissions);
TLMap(x+1) = mean(timeloss);
Fitness(x) = (TLMap(1)- TLMap(i))/TLMap(1) * 100 + 2 * ((EMMap(1)- EMMap(i))/EMMap(1)) - MaxDecelNum
if mean(timeloss) <= min(TLMap)
   ...
   deadEnd = deadEnd + 1
    ...
    previousvLeft = vLeft;
if Fitness(x+1) < min(Fitness) + tolerance   
...
plot(TLMap)
axis([0 550 910 960])
title("Timeloss progression over 500 iterations with Progression Factor of 20")
